...
export async function invokeNodeWithMediaHandling(nodeId, command, params, timeoutMs = 30_000) {
    broadcast({ type: 'node_exec', payload: { action: 'started', nodeId, command } });
    ...
    try {
        ...
        broadcast({ type: 'node_exec', payload: { action: 'completed', nodeId, command, ok: true, durationMs: Date.now() - startAt } });
        return result;
    }
    catch (e) {
        broadcast({ type: 'node_exec', payload: { action: 'completed', nodeId, command, ok: false, durationMs: Date.now() - startAt } });
        throw e;
    }
}
...
const TOOL_TO_COMMAND = {
    ...
    uiLaunch: 'ui.launch',
    uiDoubleTap: 'ui.doubleTap',
    uiWait: 'ui.wait',
    uiTakeOver: 'ui.takeOver',
    uiListApps: 'ui.listApps',
    uiTapByImage: 'ui.tapByImage',
    screenOcr: 'screen.ocr',
    screenFindImage: 'screen.findImage',
...
const COMMAND_TO_CAPABILITY = {
    ...
    'ui.launch': 'ui.launch',
    'ui.doubleTap': 'ui.doubleTap',
    'ui.wait': 'ui.wait',
    'ui.takeOver': 'ui.takeOver',
    'ui.listApps': 'ui.listApps',
    'ui.tapByImage': 'ui.tapByImage',
    'screen.ocr': 'screen.ocr',
    'screen.findImage': 'screen.findImage',
...
function getTimeoutForCommand(command, params, defaultMs) {
    ...
    if (command === 'location.get')
        return 15_000;
    if (command.startsWith('ui.') || command === 'screen.ocr' || command === 'screen.findImage') {
        if (command === 'ui.sequence' || command === 'ui.flow')
            return 60_000;
        if (command === 'ui.wait') {
            const duration = Number(params.duration ?? 1) || 1;
            return Math.min(70_000, (duration + 5) * 1000);
        }
        return 15_000;
    }
...
export async function executeNodeTool(toolId, params, execContext, timeoutMs = 30_000) {
    ...
    try {
        ...
        broadcast({ type: 'node_exec', payload: { action: 'completed', nodeId, command, ok: true, durationMs } });
        addAudit({
            type: 'node',
            action: 'invoke',
            detail: {
        ...
        const durationMs = Date.now() - startAt;
        broadcast({ type: 'node_exec', payload: { action: 'completed', nodeId, command, ok: false, durationMs } });
        addAudit({
            type: 'node',
            action: 'invoke',
            detail: {
