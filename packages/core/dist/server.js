...
export async function processChannelEvent(channel, message, ctx) {
    ...
    const { getChannelDefaultAgentId, getChannelMentionEnabled } = await import('./config/loader.js');
    ...
    console.log('[ApexPanda] parseAgentMention 完成', { agentId: mentionAgentId ?? defaultAgentId, agentIds: mentionAgentIds.length, contentLen: content.length });
    const agentId = mentionAgentId ?? defaultAgentId;
    ...
    // 多 Agent 协同：@ 2+ 个 Agent 时走 Orchestrator
    ...
    await appendToSession(channelSessionId, 'assistant', rawReply, chatTenantId);
    const reply = unmappedMention && unmappedMention.trim()
        ? `未找到 @${unmappedMention}，已使用默认助手。\n\n${rawReply}`
        : rawReply;
    ...
    if (channel === 'feishu' && ctx.messageId) {
        ...
        await sendFeishuReply(ctx.messageId, reply);
    }
        ...
        if (corpId && agentId && secret) {
            await sendWeComMessage(ctx.chatId, reply, { corpId, agentId, secret });
        }
        else {
            console.log('[wecom] Reply (no corpId/agentId/secret):', reply.slice(0, 80));
        }
    }
...
async function processFeishuEvent(result) {
    ...
    if (result.messageId && result.message.content?.includes('[语音识别失败，Agent 兜底')) {
        try {
            ...
            await sendFeishuReply(result.messageId, getVoiceFallbackUserPrompt(reason));
        }
        catch (e) {
            console.warn('[Feishu] 发送兜底提示失败', e);
        }
    }
    await processChannelEvent('feishu', result.message, {
...
export async function processFeishuEventDeferred(rawResult) {
    const { isDuplicateFeishuMessage } = await import('./channels/feishu-ws.js');
    if (rawResult.messageId && isDuplicateFeishuMessage(rawResult.messageId)) {
        console.log(`[Feishu] 跳过重复消息(deferred) ${rawResult.messageId}`);
        return;
    }
    const { parseFeishuEvent } = await import('./channels/feishu.js');
    const msg = await parseFeishuEvent(rawResult.rawBody, 'default');
    if (!msg)
        return;
...
export async function createServer() {
    const server = createHttpServer(async (req, res) => {
        ...
        // Webhooks (渠道回调)
        if (pathname === '/webhooks/dingtalk' && req.method === 'POST') {
            let body = '';
            for await (const chunk of req)
                body += chunk;
            try {
                const data = JSON.parse(body);
                const { handleDingTalkWebhook } = await import('./channels/dingtalk.js');
                const result = handleDingTalkWebhook(data);
                res.setHeader('Content-Type', 'application/json');
                res.writeHead(200);
                res.end('{}');
                if (result?.type === 'event' && result.sessionWebhook) {
                    processChannelEvent('dingtalk', result.message, {
                        sessionWebhook: result.sessionWebhook,
                    }).catch((e) => console.error('[DingTalk]', e));
                }
            }
            catch {
                res.writeHead(400);
        ...
        if (pathname === '/webhooks/wecom' && req.method === 'GET') {
            // 企业微信 URL 验证：配置回调时发送 GET 请求，需解密 echostr 后原样返回
            try {
                const { getWeComToken, getWeComEncodingAESKey } = await import('./config/loader.js');
                const { handleWeComVerify } = await import('./channels/wecom.js');
                ...
                const encodingAESKey = getWeComEncodingAESKey();
                const plain = handleWeComVerify({ token, encodingAESKey, msgSignature, timestamp, nonce, echostr });
                if (plain) {
                    ...
                    console.warn('[WeCom] URL 验证失败，请检查 Token 和 EncodingAESKey 配置是否与企业微信后台一致');
                    res.writeHead(403);
                    res.end('Verification failed');
                }
            }
            catch (e) {
                console.error('[WeCom] URL 验证异常:', e);
                res.writeHead(500);
                res.end('Error');
            }
            return;
        }
        if (pathname === '/webhooks/wecom' && req.method === 'POST') {
            let body = '';
            for await (const chunk of req)
                body += chunk;
            try {
                const { handleWeComWebhook } = await import('./channels/wecom.js');
                const result = handleWeComWebhook(body);
                res.setHeader('Content-Type', 'text/plain');
                res.writeHead(200);
                res.end('');
                if (result?.type === 'event') {
                    processChannelEvent('wecom', result.message, {
        ...
        if ((pathname === '/webhooks/feishu' || pathname === '/webhooks/lark') && req.method === 'POST') {
            let body = '';
            for await (const chunk of req)
                body += chunk;
            try {
                ...
                const { isChannelConfigured } = await import('./config/loader.js');
                ...
                if (result?.type === 'challenge') {
                    ...
                    res.writeHead(200);
                    res.end('{}');
                    // 方案 A：已启用 WebSocket 长连接时，忽略 HTTP 事件订阅，避免重复回复
        ...
        if (pathname === '/webhooks/telegram' && req.method === 'POST') {
            ...
            try {
                ...
                res.end('{}');
                if (result?.type === 'event') {
                    processChannelEvent('telegram', result.message, {
                        chatId: result.chatId,
        ...
        if (pathname === '/webhooks/slack' && req.method === 'POST') {
            let body = '';
            for await (const chunk of req)
                body += chunk;
            try {
                const { getSlackSigningSecret } = await import('./config/loader.js');
                ...
                if (result?.type === 'challenge') {
                    ...
                    res.writeHead(200);
                    res.end('{}');
                    processChannelEvent('slack', result.message, {
        ...
        if (pathname === '/webhooks/whatsapp') {
            const { getWhatsAppVerifyToken } = await import('./config/loader.js');
            ...
            if (req.method === 'POST') {
                ...
                try {
                    ...
                    res.end('{}');
                    if (result?.type === 'event') {
                        const { getWhatsAppPhoneNumberId } = await import('./config/loader.js');
                        processChannelEvent('whatsapp', result.message, {
                            chatId: result.message.channelPeerId,
                            phoneNumberId: result.phoneNumberId ?? getWhatsAppPhoneNumberId(),
...
async function handleApi(req, res, pathname) {
    ...
    if (pathname === '/api/v1/channels' && method === 'GET') {
        const { isChannelConfigured, hasChannelCredentialsForUi, getConfigSync, isSecretsFromEnvOnly } = await import('./config/loader.js');
        ...
        const channels = [
            { id: 'feishu', name: '飞书', connectionMode: 'ws', webhookPath: '/webhooks/feishu', configFields: ['appId', 'appSecret', 'defaultAgentId', 'mentionEnabled'] },
            { id: 'lark', name: 'Lark（飞书国际版）', connectionMode: 'ws', webhookPath: '/webhooks/lark', configFields: ['appId', 'appSecret', 'defaultAgentId', 'mentionEnabled'] },
            { id: 'dingtalk', name: '钉钉', connectionMode: 'webhook', webhookPath: '/webhooks/dingtalk', configFields: ['webhookUrl', 'defaultAgentId', 'mentionEnabled'] },
            { id: 'wecom', name: '企业微信', connectionMode: 'webhook', webhookPath: '/webhooks/wecom', configFields: ['corpId', 'agentId', 'secret', 'token', 'encodingAESKey', 'defaultAgentId', 'mentionEnabled'] },
            { id: 'telegram', name: 'Telegram', connectionMode: 'ws', webhookPath: '/webhooks/telegram', configFields: ['botToken', 'defaultAgentId', 'mentionEnabled'] },
            { id: 'slack', name: 'Slack', connectionMode: 'ws', webhookPath: '/webhooks/slack', configFields: ['botToken', 'appToken', 'signingSecret', 'defaultAgentId', 'mentionEnabled'] },
            { id: 'whatsapp', name: 'WhatsApp', connectionMode: 'webhook', webhookPath: '/webhooks/whatsapp', configFields: ['verifyToken', 'accessToken', 'phoneNumberId', 'defaultAgentId', 'mentionEnabled'] },
            { id: 'discord', name: 'Discord', connectionMode: 'ws', webhookPath: '/webhooks/discord', configFields: ['botToken', 'defaultAgentId', 'mentionEnabled'] },
        ];
        const list = channels.map((ch) => {
            const hasCredentials = hasChannelCredentialsForUi(ch.id);
            const enabled = isChannelConfigured(ch.id);
            const stored = cfg.channels?.[ch.id];
            const config = {};
            for (const f of ch.configFields) {
                const v = stored?.[f];
            ...
            return { ...ch, enabled, hasCredentials, config };
        });
        res.setHeader('Content-Type', 'application/json');
        res.writeHead(200);
        res.end(JSON.stringify({ channels: list, baseUrl: baseUrl || undefined, secretsEditable }));
        return;
    }
    const channelsPatchMatch = pathname?.match(/^\/api\/v1\/channels\/([a-z0-9_-]+)$/);
    if (channelsPatchMatch && method === 'PATCH') {
        const channelId = channelsPatchMatch[1];
        const validIds = ['feishu', 'lark', 'dingtalk', 'wecom', 'telegram', 'slack', 'whatsapp', 'discord'];
        if (!validIds.includes(channelId)) {
            res.setHeader('Content-Type', 'application/json');
            res.writeHead(404);
        ...
        try {
            ...
            const ch = (await loadConfig()).channels?.[channelId] ?? {};
            const next = { ...ch };
            if (patch.defaultAgentId !== undefined) {
                next.defaultAgentId = typeof patch.defaultAgentId === 'string' ? patch.defaultAgentId.trim() || undefined : undefined;
            }
            if (patch.mentionEnabled !== undefined) {
                next.mentionEnabled = patch.mentionEnabled === true;
            }
            ...
            if (channelId === 'feishu' || channelId === 'lark') {
                ...
                if (typeof patch.appSecret === 'string' && !isSecretsFromEnvOnly())
                    next.appSecret = patch.appSecret;
            }
            else if (channelId === 'telegram') {
                if (typeof patch.botToken === 'string' && !isSecretsFromEnvOnly())
                    next.botToken = patch.botToken;
            }
            else if (channelId === 'slack') {
                if (typeof patch.botToken === 'string' && !isSecretsFromEnvOnly())
                    next.botToken = patch.botToken;
                if (typeof patch.appToken === 'string' && !isSecretsFromEnvOnly())
                    next.appToken = patch.appToken;
                if (typeof patch.signingSecret === 'string' && !isSecretsFromEnvOnly())
                    next.signingSecret = patch.signingSecret;
            }
            else if (channelId === 'whatsapp') {
                if (typeof patch.verifyToken === 'string' && !isSecretsFromEnvOnly())
                    next.verifyToken = patch.verifyToken;
                if (typeof patch.accessToken === 'string' && !isSecretsFromEnvOnly())
                    next.accessToken = patch.accessToken;
                if (typeof patch.phoneNumberId === 'string')
                    next.phoneNumberId = patch.phoneNumberId;
            }
                ...
                if (typeof patch.corpId === 'string')
                    next.corpId = patch.corpId;
                if (typeof patch.agentId === 'string')
                    next.agentId = patch.agentId;
                if (typeof patch.secret === 'string' && !isSecretsFromEnvOnly())
                    next.secret = patch.secret;
                if (typeof patch.token === 'string')
                    next.token = patch.token;
                if (typeof patch.encodingAESKey === 'string')
                    next.encodingAESKey = patch.encodingAESKey;
            }
            ...
            console.log('[ApexPanda] 渠道配置已更新:', names[channelId] ?? channelId);
            addAudit({ type: 'channel', action: 'update', detail: { channelId } });
            res.setHeader('Content-Type', 'application/json');
            res.writeHead(200);
            res.end(JSON.stringify({ ok: true }));
        }
    ...
    if (channelsDeleteMatch && method === 'DELETE') {
        const channelId = channelsDeleteMatch[1];
        const validIds = ['feishu', 'lark', 'dingtalk', 'wecom', 'telegram', 'slack', 'whatsapp', 'discord'];
        if (!validIds.includes(channelId)) {
            res.setHeader('Content-Type', 'application/json');
            res.writeHead(404);
        ...
        try {
            ...
            const names = { feishu: '飞书', lark: 'Lark', telegram: 'Telegram', slack: 'Slack', discord: 'Discord', dingtalk: '钉钉', wecom: '企业微信', whatsapp: 'WhatsApp' };
            console.log('[ApexPanda] 渠道配置已清空:', names[channelId] ?? channelId);
            addAudit({ type: 'channel', action: 'delete', detail: { channelId } });
            res.setHeader('Content-Type', 'application/json');
            res.writeHead(200);
            res.end(JSON.stringify({ ok: true }));
        }
            ...
            res.setHeader('Content-Type', 'application/json');
            res.writeHead(400);
            res.end(JSON.stringify({ error: e instanceof Error ? e.message : 'Clear failed' }));
        }
        return;
    }
    if (pathname === '/api/v1/nodes' && method === 'GET') {
    ...
    if (pathname === '/api/v1/config' && method === 'GET') {
        const { getConfigSync, getLLMBaseUrl, getLLMModel, getLLMFallbackModel, getWorkspaceDir, isSecretsFromEnvOnly, getConfigPath, loadConfig, getDiscussionConfig, getMemoryConfig, getDeleteConfirmRequired, getHybridSearchEnabled, DEFAULT_INTENT_MAPPINGS, } = await import('./config/loader.js');
        const cfg = getConfigSync();
    ...
    if (pathname === '/api/v1/sessions' && method === 'GET') {
        ...
        let filtered = data;
        if (channel) {
            const infer = (id) => id.startsWith('ch:feishu:') ? 'feishu' : id.startsWith('ch:lark:') ? 'lark'
                : id.startsWith('ch:dingtalk:') ? 'dingtalk' : id.startsWith('ch:wecom:') ? 'wecom'
                    : id.startsWith('ch:telegram:') ? 'telegram' : id.startsWith('ch:slack:') ? 'slack'
                        : id.startsWith('ch:discord:') ? 'discord' : id.startsWith('ch:whatsapp:') ? 'whatsapp' : id.startsWith('chat-') ? 'chat'
                            : id.startsWith('default-') ? 'api' : 'other';
            filtered = data.filter((s) => (s.meta?.channel ?? infer(s.id)) === channel);
        }
        res.setHeader('Content-Type', 'application/json');
        res.writeHead(200);
    ...
    if (pathname === '/api/v1/install' && method === 'POST') {
        ...
        try {
            ...
            try {
                const { loadConfig, saveConfig, invalidateConfigCache, getConfigPath } = await import('./config/loader.js');
                ...
                const channels = {};
                for (const [id, fields] of Object.entries(payload.channels ?? {})) {
                    if (fields && typeof fields === 'object' && Object.values(fields).some(Boolean)) {
                        channels[id] = fields;
                    }
                }
