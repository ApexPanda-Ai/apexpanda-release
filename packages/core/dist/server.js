...
export async function processChannelEvent(channel, message, ctx) {
    ...
    if (getDeleteConfirmRequired()) {
        ...
        if (pending && (isConfirmIntent(msgContent) || isCancelIntent(msgContent))) {
            ...
            const sessionMeta = { channel, userId: ctx.userId, peer: ctx.chatId ?? ctx.messageId ?? ctx.sessionWebhook };
            if (isCancelIntent(msgContent)) {
                await sendReplyToChannel(channel, ctx, '已取消删除');
                await appendToSession(channelSessionId, 'user', msgContent, chatTenantId, sessionMeta);
                await appendToSession(channelSessionId, 'assistant', '已取消删除', chatTenantId);
            }
                ...
                const reply = result.ok ? '已执行删除' : `删除失败：${result.error}`;
                await sendReplyToChannel(channel, ctx, reply);
                await appendToSession(channelSessionId, 'user', msgContent, chatTenantId, sessionMeta);
                await appendToSession(channelSessionId, 'assistant', reply, chatTenantId);
            }
            else {
                const result = await executePendingDelete(pending.path, pending.workspaceDir);
                const reply = result.ok ? `已删除 ${pending.path}` : `删除失败：${result.error}`;
                await sendReplyToChannel(channel, ctx, reply);
                await appendToSession(channelSessionId, 'user', msgContent, chatTenantId, sessionMeta);
                await appendToSession(channelSessionId, 'assistant', reply, chatTenantId);
            }
    ...
    if (cancelAutoMatch) {
        const { setSessionAutoApprove, appendToSession } = await import('./session/store.js');
        await setSessionAutoApprove(channelSessionId, false);
        const reply = '已关闭自动执行模式，节点命令将恢复为需审批。';
        await sendReplyToChannel(channel, ctx, reply);
        const chatTenantId = channel === 'chat' ? ctx.tenantId : undefined;
        await appendToSession(channelSessionId, 'user', msgContent, chatTenantId, { channel, userId: ctx.userId, peer: ctx.chatId ?? ctx.messageId ?? ctx.sessionWebhook });
        await appendToSession(channelSessionId, 'assistant', reply, chatTenantId);
        return;
    }
    ...
    // 多 Agent 动态规划待确认：检查是否有 pending plan，「确认」则执行，「取消」则清除
    {
        ...
        if (await hasPendingPlan(channelSessionId)) {
            ...
            if (isPlanCancelMessage(rawMsg)) {
                ...
                await sendReplyToChannel(channel, ctx, reply);
                const chatTenantId = channel === 'chat' ? ctx.tenantId : undefined;
                await appendToSession(channelSessionId, 'user', rawMsg, chatTenantId, { channel, userId: ctx.userId, peer: ctx.chatId ?? ctx.messageId ?? ctx.sessionWebhook });
                await appendToSession(channelSessionId, 'assistant', reply, chatTenantId);
                return;
            }
