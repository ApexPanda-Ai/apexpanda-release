...
export async function runAgent(config, input) {
    ...
    if (knowledgeStore) {
        try {
            ...
            console.warn('[Agent] 知识库检索失败，将基于对话历史继续回答:', e instanceof Error ? e.message : e);
            ragChunks = [];
            context = '';
        }
    }
...
【文件直发】用户明确要求「发给我」「直接发到群里/渠道」「把这个文件发过来」时，write_file/create_file 必须传 deliverToChannel:true，文件将自动发送到飞书/Telegram 等当前渠道。适用于 txt、md、csv、json、pdf、doc、docx 等文档；单文件不超过 10MB。
${fullControl
