...
export type WorkflowChannelContext = {
    ...
    userId?: string;
    /** 企业微信智能机器人：WebSocket frame，用于 replyText */
    wecomFrame?: unknown;
};
export type WorkflowNodeType = 'agent' | 'skill' | 'human' | 'loop' | 'mcp' | 'verify';
export interface WorkflowNode {
     ...
     * }
     * verify: {
     *   checkpoint: string,       // 校验规则描述（如「必须包含数字、无逻辑矛盾」）
     *   validator: 'llm' | 'rule' | 'skill',
     *   skillName?: string,       // validator='skill' 时必填
     *   toolId?: string,          // validator='skill' 时必填
     *   params?: Record<string, unknown>,
     *   keywords?: string[],      // validator='rule' 时：任一关键词存在则通过
     *   regex?: string,           // validator='rule' 时：正则匹配则通过
     * }
     */
