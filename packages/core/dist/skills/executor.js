...
import { basename, dirname, extname, join, relative as pathRelative, resolve, sep } from 'node:path';
...
const builtinHandlers = {
    ...
    'file-tools#writeFile': async ({ path: p, content, deliverToChannel }, ctx) => {
        const ws = ctx?.workspaceDir ?? getWorkspaceDir();
        const fp = resolveWorkspacePath(ws, String(p ?? ''));
        await mkdir(dirname(fp), { recursive: true });
        const data = content != null && typeof content === 'string' ? content : String(content ?? '');
        await writeFile(fp, data);
        const base = { path: fp, written: true };
        const shouldDeliver = deliverToChannel === true || deliverToChannel === 'true';
        if (!shouldDeliver)
            return base;
        const DELIVER_MAX_BYTES = 10 * 1024 * 1024;
        let st;
        try {
            st = await stat(fp);
            if (!st.isFile() || st.size <= 0)
                return base;
            if (st.size > DELIVER_MAX_BYTES) {
                return { ...base, _fileReply: null, note: `文件超过 ${(DELIVER_MAX_BYTES / 1024 / 1024).toFixed(0)}MB，仅保存未发送` };
            }
        }
        catch {
            return base;
        ...
        const fileType = imageExts.includes(ext) ? 'image' : audioExts.includes(ext) ? 'audio' : videoExts.includes(ext) ? 'video' : 'file';
        const mimeMap = {
            txt: 'text/plain', md: 'text/markdown', json: 'application/json', csv: 'text/csv', html: 'text/html', xml: 'application/xml',
            png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', gif: 'image/gif', webp: 'image/webp',
            pdf: 'application/pdf', doc: 'application/msword', docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
            xls: 'application/vnd.ms-excel', xlsx: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            ppt: 'application/vnd.ms-powerpoint', pptx: 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
            mp3: 'audio/mpeg', wav: 'audio/wav', ogg: 'audio/ogg', mp4: 'video/mp4', zip: 'application/zip',
        };
    ...
    // dingtalk-message（钉钉群机器人推送，Stream 模式下需传入 sessionWebhook）
    'dingtalk-message#send': async ({ webhookUrl, content }) => {
        const url = String(webhookUrl ?? '').trim();
        if (!url)
            throw new Error('钉钉 Stream 模式需传入 webhookUrl（如 cronOutput.sessionWebhook）');
    ...
    // wecom-message（企业微信智能机器人推送，需 botId+secret 长连接）
    'wecom-message#send': async ({ userId, content }) => {
        ...
        const wecomInst = instances.find((i) => i.type === 'wecom' && isChannelConfigured(i.id));
        if (!wecomInst)
            throw new Error('企业微信需配置 botId、secret（智能机器人长连接模式）');
        ...
        await sendWecomBotText(wecomInst.id, uid, text);
        return { sent: true };
    },
