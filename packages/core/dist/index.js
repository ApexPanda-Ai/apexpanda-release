...
import { getConfigSync, loadConfig, startConfigWatch, isChannelConfigured, configReloadEmitter, logConfigSummary } from './config/loader.js';
...
async function startChannelsIfConfigured() {
    if (isChannelConfigured('feishu')) {
        console.log('[ApexPanda] 正在启动渠道: 飞书');
        try {
            startFeishuWebSocket();
        }
        catch (e) {
            console.error('[ApexPanda] 渠道 飞书 启动失败:', e);
        }
    }
    if (isChannelConfigured('telegram')) {
        console.log('[ApexPanda] 正在启动渠道: Telegram');
        const { startTelegramPolling } = await import('./channels/telegram-ws.js');
        startTelegramPolling().catch((e) => console.error('[ApexPanda] 渠道 Telegram 启动失败:', e));
    }
    if (isChannelConfigured('slack')) {
        console.log('[ApexPanda] 正在启动渠道: Slack');
        const { startSlackSocket } = await import('./channels/slack-ws.js');
        startSlackSocket().catch((e) => console.error('[ApexPanda] 渠道 Slack 启动失败:', e));
    }
    if (isChannelConfigured('discord')) {
        console.log('[ApexPanda] 正在启动渠道: Discord');
        const { startDiscordClient } = await import('./channels/discord-ws.js');
        startDiscordClient().catch((e) => console.error('[ApexPanda] 渠道 Discord 启动失败:', e));
    }
}
async function main() {
