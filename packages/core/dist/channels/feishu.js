 ...
 * @param instanceId 方案 B：多实例时传入实例 ID，用于 getFeishuMessageResource 等 */
export async function parseFeishuEvent(body, tenantId, instanceId) {
    if (body.challenge)
        return null;
    ...
    if (messageType === 'image') {
        try {
            ...
            const buf = await getFeishuMessageResource(msg.message_id, imageKey, 'image', instanceId);
        ...
        try {
            ...
            const { mkdir, writeFile } = await import('node:fs/promises');
            const { join } = await import('node:path');
            const buf = await getFeishuMessageResource(msg.message_id, fileKey, 'file', instanceId);
        ...
        try {
            ...
            const result = await runUnrecognizedFallback('voice', { fileKey, messageId: msg.message_id, instanceId }, {
                input: { fileKey, messageId: msg.message_id, instanceId },
            });
