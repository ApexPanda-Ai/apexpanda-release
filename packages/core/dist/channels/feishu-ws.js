 ...
 * 方案 B：支持多实例，每个 instanceId 独立 WS 连接
 */
import * as Lark from '@larksuiteoapi/node-sdk';
import { getChannelConfig } from '../config/loader.js';
import { logMem } from '../debug-mem.js';
import { enqueueFeishuJob, registerMemoryEnqueue, } from './channel-queue.js';
/** 方案 B：按 instanceId 维护多个 WS 客户端 */
const wsClients = new Map();
let _memoryEnqueueRegistered = false;
/**
 * 已处理的 message_id 去重表
...
/**
 * 队列全局最大容量。超出时丢弃新消息，防止高并发下内存无界增长（OOM）。
 * 可通过 APEXPANDA_FEISHU_QUEUE_MAX 环境变量覆盖，默认 200 条。
 */
const QUEUE_MAX_SIZE = (() => {
    const v = parseInt(process.env.APEXPANDA_FEISHU_QUEUE_MAX ?? '', 10);
    return Number.isFinite(v) && v > 0 ? v : 200;
})();
/**
 * 单个 chatId 在队列中的最大积压条数。
 * 防止一个会话刷屏独占整条队列，饥饿其他会话。默认 20 条。
 */
const QUEUE_MAX_PER_CHAT = (() => {
    const v = parseInt(process.env.APEXPANDA_FEISHU_QUEUE_MAX_PER_CHAT ?? '', 10);
    return Number.isFinite(v) && v > 0 ? v : 20;
})();
/**
 * 单个任务最大处理时长（毫秒）。
 * 超时后强制释放 chatId 占位，防止后续消息永久阻塞。
 * 可通过 APEXPANDA_TASK_TIMEOUT_MS 环境变量覆盖，默认 10 分钟。
 */
...
function processQueue() {
    ...
    // 守卫超时：无论任务是否完成，到期后强制释放 chatId，确保同会话后续消息不被永久阻塞。
    // 任务本身在后台继续运行，由 LLM 层自身的超时（APEXPANDA_LLM_TIMEOUT_MS）兜底终止。
    let guardTimer = null;
    const guardPromise = new Promise((_, reject) => {
        guardTimer = setTimeout(() => {
            reject(new Error(`任务超时（${TASK_PROCESS_TIMEOUT_MS / 1000}s），已强制释放 chatId=${chatId}`));
        }, TASK_PROCESS_TIMEOUT_MS);
    });
    Promise.race([handleFeishuMessageSync(next.instanceId, next.event), guardPromise])
        .catch((e) => {
        const msg = e instanceof Error ? e.message : String(e);
        if (msg.includes('已强制释放')) {
            console.warn(`[Feishu WS] ${msg}`);
        }
        else {
            console.error('[Feishu WS] queue processor 异常:', e);
        }
    })
        .finally(() => {
        if (guardTimer !== null)
...
export async function handleFeishuMessageSync(instanceId, event) {
    ...
    if (messageType === 'audio') {
        ...
        if (result.error) {
            ...
            if (result.savedPath) {
                ...
                try {
                    ...
                    await sendFeishuReply(messageId, userPrompt, instanceId);
                }
                catch (e) {
        ...
        try {
            ...
            const buf = await getFeishuMessageResource(messageId, imageKey, 'image', instanceId);
        ...
        try {
            ...
            const { mkdir, writeFile } = await import('node:fs/promises');
            const { join } = await import('node:path');
            const buf = await getFeishuMessageResource(messageId, fileKey, 'file', instanceId);
    ...
    await processChannelEvent(instanceId, { content: text }, {
        messageId,
        chatId,
        chatType,
...
function handleFeishuMessage(instanceId, event) {
    void enqueueFeishuJob({ kind: 'ws', instanceId, event });
}
function pushToMemoryQueue(payload) {
    // 全局容量检查：超出上限直接丢弃，避免无界增长导致 OOM
    if (channelEventQueue.length >= QUEUE_MAX_SIZE) {
        const msgId = payload.event.message?.message_id ?? '?';
        console.warn(`[Feishu WS] 队列已满（${QUEUE_MAX_SIZE}），丢弃消息 ${msgId}（当前积压 ${channelEventQueue.length} 条）`);
        return;
    }
    // 单 chatId 容量检查：防止一个会话刷屏独占队列，饥饿其他会话
    const chatId = payload.event.message?.chat_id ?? '';
    if (chatId) {
        const chatCount = channelEventQueue.filter((item) => item.event.message?.chat_id === chatId).length;
        if (chatCount >= QUEUE_MAX_PER_CHAT) {
            const msgId = payload.event.message?.message_id ?? '?';
            console.warn(`[Feishu WS] chatId=${chatId} 积压已达上限（${QUEUE_MAX_PER_CHAT}），丢弃消息 ${msgId}`);
            return;
        }
    }
    channelEventQueue.push({ instanceId: payload.instanceId, event: payload.event });
    setImmediate(() => processQueue());
}
/** 方案 B：按 instanceId 启动飞书 WebSocket，使用该实例的凭证 */
export function startFeishuWebSocket(instanceId) {
    const cfg = getChannelConfig(instanceId);
    const appId = cfg?.appId?.trim();
    const appSecret = cfg?.appSecret?.trim();
    if (!appId || !appSecret)
        return;
    if (!_memoryEnqueueRegistered) {
        _memoryEnqueueRegistered = true;
        registerMemoryEnqueue((p) => {
            if (p.kind === 'ws')
                pushToMemoryQueue(p);
        });
    }
    if (wsClients.has(instanceId)) {
        console.log('[ApexPanda] 飞书 WebSocket 已在运行，跳过:', instanceId);
        return;
    }
    ...
    eventDispatcher.register({
        'im.message.receive_v1': (data) => {
            try {
                const event = data;
                handleFeishuMessage(instanceId, event);
            }
            catch (e) {
    ...
    wsClients.set(instanceId, client);
    logMem('feishu-ws:after-client.start');
    console.log('[ApexPanda] 飞书 WebSocket 已启动:', instanceId);
}
/** 方案 B：按 instanceId 停止指定实例 */
export function stopFeishuWebSocket(instanceId) {
    const client = wsClients.get(instanceId);
    if (client) {
        try {
            client.stop?.();
        }
        ...
        console.log('[ApexPanda] 飞书 WebSocket 已停止:', instanceId);
    }
}
/** 方案 B：停止所有飞书实例（配置重载时调用） */
export function stopAllFeishuWebSockets() {
    for (const id of wsClients.keys()) {
        stopFeishuWebSocket(id);
    }
}
