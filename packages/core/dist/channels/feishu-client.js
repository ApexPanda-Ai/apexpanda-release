...
export async function getFeishuTenantAccessToken() {
    ...
    const { getFeishuAppId, getFeishuAppSecret } = await import('../config/loader.js');
...
export async function sendFeishuMessage(options) {
...
export async function sendFeishuReply(messageId, content) {
    const token = await getFeishuTenantAccessToken();
...
/** 发送 /help 帮助卡片 */
export async function sendFeishuHelpCard(messageId, helpText) {
...
export async function sendFeishuReplyCard(messageId, card) {
    const token = await getFeishuTenantAccessToken();
    const res = await fetchWithRetry(`${FEISHU_API}/im/v1/messages/${messageId}/reply`, {
        method: 'POST',
        headers: {
 ...
 */
export async function getFeishuMessageResource(messageId, fileKey, resourceType = 'file') {
    const token = await getFeishuTenantAccessToken();
...
export async function recognizeFeishuSpeech(options) {
    const { getFeishuAppId, getFeishuAppSecret, getWorkspaceDir } = await import('../config/loader.js');
    const appId = getFeishuAppId();
    const appSecret = getFeishuAppSecret();
    if (!appId || !appSecret) {
 ...
 */
export async function replyFeishuImage(messageId, filePath, caption) {
    ...
    // 如果有说明文字，追加发一条文本回复
    if (caption) {
        await sendFeishuReply(messageId, caption);
    }
}
/**
 * 上传文件到飞书并回复文件消息
 ...
 */
export async function replyFeishuFile(messageId, filePath, mimeType, caption) {
    const { readFile } = await import('node:fs/promises');
