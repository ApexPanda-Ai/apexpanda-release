...
import { getRunCheckpoint } from '../workflow/store.js';
import { getKnowledgeStore } from '../knowledge/store-getter.js';
import { getMultiAgentConfig, getVerifyConfig } from '../config/loader.js';
...
import { appendMultiAgentRun, updateMultiAgentRun, makeRunId } from './multi-agent-run-store.js';
...
/** 阶段三：从 checkpoint 构建步骤列表（含 verify 通过/失败）供沙盘展示 */
async function buildRunSteps(def, runId, workflowStatus) {
    const cp = await getRunCheckpoint(runId);
    const stepOutputs = cp?.stepOutputs ?? {};
    const levels = topologicalLevelsFromDef(def);
    const flatOrder = levels.flat();
    const failedNodeId = workflowStatus === 'failed'
        ? flatOrder.find((id) => stepOutputs[id] == null)
        : undefined;
    return def.nodes.map((n) => {
        const hasOutput = stepOutputs[n.id] != null;
        const isFailed = n.id === failedNodeId;
        const status = hasOutput ? 'completed' : isFailed ? 'failed' : 'pending';
        const label = n.type === 'agent'
            ? n.config?.message?.slice(0, 30) ?? n.id
            : n.type === 'verify'
                ? `校验 ${n.config?.checkpoint?.slice(0, 20) ?? ''}`
                : n.type === 'loop'
                    ? `循环 ${n.config?.exitCondition ?? ''}`
                    : n.id;
        const pass = n.type === 'verify' ? (hasOutput ? true : isFailed ? false : undefined) : undefined;
        return { id: n.id, type: n.type, label, status, pass };
    });
}
function topologicalLevelsFromDef(def) {
...
function formatPlanPreview(plan, agentById) {
    const lines = ['【执行计划预览】\n'];
    if (getVerifyConfig().enabled) {
        lines.push('✓ 已启用校验（每步产出将经 LLM 验证）\n');
    }
...
async function runPlanOrchestrator(input, agents, channel, ctx, prebuiltPlan) {
    ...
    if (cfg.planConfirmRequired && !prebuiltPlan) {
        ...
        await setPendingPlan(input.channelSessionId, {
            input, agents, preview, channel, ctx, createdAt: Date.now(),
            runId: planInput._runId, mode: planInput._mode, task: input.task, agentIds: input.agentIds, agentNames: agents.map((a) => a.name),
        });
        return { reply: preview };
    }
    const runId = planInput._runId ?? makeRunId();
    const agentNames = agents.map((a) => a.name);
    const agentIds = input.agentIds ?? agents.map((a) => a.id);
    const mode = planInput._mode ?? 'plan';
    const startedAt = Date.now();
    if (planInput._runId) {
        updateMultiAgentRun(runId, { status: 'running' }).catch(() => { });
    }
    else {
        appendMultiAgentRun({ runId, mode, task, agentNames, agentIds, status: 'running', channel, startedAt, autoSelectReason: input.autoSelectReason }).catch(() => { });
    }
    broadcast({ type: 'multi_agent_run', payload: { action: 'started', runId, mode, task, agentIds, agentNames, startedAt } });
    ...
    // 阶段三：Verify 节点（verifyEnabled 时在每个 agent 步骤后插入）
    const verifyCfg = getVerifyConfig();
    const stepIdsInLoops = new Set((plan.loops ?? []).flatMap((lp) => lp.steps));
    if (verifyCfg.enabled) {
        for (const s of steps) {
            if (stepIdsInLoops.has(s.id))
                continue; // loop 内步骤暂不插入 verify
            const verifyId = `verify_${s.id}`;
            nodes.push({
                id: verifyId,
                type: 'verify',
                config: {
                    checkpoint: `产出需完整、与子任务「${s.subTask}」一致、无逻辑矛盾`,
                    validator: 'llm',
                },
            });
        }
    ...
    // 边：agent 依赖关系；若有 verify，则下游依赖 verify 节点而非 agent
    const edges = [];
    const resolveFrom = (fromId) => {
        return verifyCfg.enabled && !stepIdsInLoops.has(fromId) ? `verify_${fromId}` : fromId;
    };
    for (const s of steps) {
        for (const fromId of s.after ?? []) {
            if (!allNodeIds.has(fromId) && !allNodeIds.has(`verify_${fromId}`))
                continue;
            edges.push({ from: resolveFrom(fromId), to: s.id });
        }
    }
    for (const lp of plan.loops ?? []) {
        for (const fromId of lp.after ?? []) {
            if (!allNodeIds.has(fromId) && !allNodeIds.has(`verify_${fromId}`))
                continue;
            edges.push({ from: resolveFrom(fromId), to: lp.id });
        }
    }
    // 每个 agent 步骤 -> 其 verify 节点
    if (verifyCfg.enabled) {
        for (const s of steps) {
            if (stepIdsInLoops.has(s.id))
                continue;
            if (allNodeIds.has(`verify_${s.id}`)) {
                edges.push({ from: s.id, to: `verify_${s.id}` });
            }
        }
    }
    const def = {
    ...
    // 阶段三：从 checkpoint 构建 runSteps（含 verify 通过/失败）供沙盘展示
    const runSteps = await buildRunSteps(def, runId, status);
    if (status === 'failed' || error) {
        const errMsg = error ?? '未知错误';
        updateMultiAgentRun(runId, { status: 'failed', completedAt, error: errMsg, steps: runSteps }).catch(() => { });
        broadcast({ type: 'multi_agent_run', payload: { action: 'completed', runId, status: 'failed', completedAt, error: errMsg } });
        return { reply: `动态规划执行失败：${errMsg}` };
    }
    const outputText = typeof output === 'string' ? output : Array.isArray(output) ? output.join('\n\n---\n\n') : JSON.stringify(output, null, 2);
    const reply = `经动态规划协作，最终产出：\n\n` + outputText;
    updateMultiAgentRun(runId, { status: 'completed', completedAt, replySummary: outputText.slice(0, 200), steps: runSteps }).catch(() => { });
    broadcast({ type: 'multi_agent_run', payload: { action: 'completed', runId, status: 'completed', completedAt, replySummary: outputText.slice(0, 200) } });
    return { reply };
}
/**
...
export async function runMultiAgentOrchestrator(channel, ctx, input) {
    ...
    const agentNames = agents.map((a) => a.name);
    const startedAt = Date.now();
    const runPayload = { runId, mode: effectiveMode, task, agentIds, agentNames, startedAt, autoSelectReason: input.autoSelectReason };
    appendMultiAgentRun({ ...runPayload, status: 'running', channel }).catch(() => { });
    broadcast({ type: 'multi_agent_run', payload: { action: 'started', ...runPayload } });
    const withLog = async (fn) => {
        try {
            const result = await fn();
            const completedAt = Date.now();
            await updateMultiAgentRun(runId, { status: 'completed', completedAt, replySummary: result.reply.slice(0, 200) });
            broadcast({ type: 'multi_agent_run', payload: { action: 'completed', runId, status: 'completed', completedAt, replySummary: result.reply.slice(0, 200) } });
            return result;
        }
        catch (e) {
            const err = e instanceof Error ? e.message : String(e);
            const completedAt = Date.now();
            await updateMultiAgentRun(runId, { status: 'failed', completedAt, error: err });
            broadcast({ type: 'multi_agent_run', payload: { action: 'completed', runId, status: 'failed', completedAt, error: err } });
            throw e;
        }
    };
    ...
    if (effectiveMode === 'plan') {
        // 待确认时先记录 pending 日志
        appendMultiAgentRun({ runId, mode: effectiveMode, task, agentNames, agentIds, status: 'pending_confirm', channel, startedAt, autoSelectReason: input.autoSelectReason }).catch(() => { });
        const planInput = { ...input, _runId: runId, _mode: effectiveMode };
        return runPlanOrchestrator(planInput, agents, channel, ctx);
    }
