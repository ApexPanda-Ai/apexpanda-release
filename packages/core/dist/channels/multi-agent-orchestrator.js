...
async function runPipelineOrchestrator(input, agents) {
    ...
    for (let i = 0; i < pipelineOrder.length; i++) {
        ...
        prevOutput = reply;
        stepReplies.push(reply);
        // 写入黑板，后续 Agent 可按名称读取
        blackboardWrite(bbId, agent.name, reply);
        accumulatedHistory = [...accumulatedHistory, { role: 'user', content: message }, { role: 'assistant', content: reply }];
    }
